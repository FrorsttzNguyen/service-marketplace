/**
 * Typed provider-bookings client — thin wrappers over apiGet/apiPut for the provider side
 * of the booking workflow.
 *
 * Mirrors the layout of `bookings.ts` (the customer side), but is intentionally a
 * SEPARATE file: the provider's view of a booking is a different use case (advancing
 * incoming requests through their lifecycle vs. the customer managing their own
 * requests), and the endpoints are different (`/api/bookings/provider` vs `/api/bookings`;
 ...
 * (`BookingResponse`), so there's no reason to redefine it. A provider booking and a
 * customer booking render the same fields; only the actions differ.
 *
 * All endpoints require a VENDOR-role JWT (the provider who owns the service the booking
 ...
 * provider); the client also gates the route via <RequireAuth requireRole="VENDOR">.
 *
 * SCOPE (per the backend contract) — the provider drives the booking lifecycle forward:
 *   - GET /api/bookings/provider?page&size  -> Page<BookingResponse> (bookings on this
 *                                            provider's services, ALL statuses)
 ...
 * as a side exit). The provider advances it one step at a time: confirm lets the customer
...
/** Spring `Page<BookingResponse>` shape for the provider's bookings. Same as customer side. */
export type ProviderBookingPage = components["schemas"]["PageBookingResponse"];

/** Pagination options for the provider's booking list. */
export interface ListProviderBookingsParams {
  page?: number; // 0-based, matches Spring's Pageable
  size?: number;
}

/**
 * List the bookings on the authenticated provider's services
 * (`GET /api/bookings/provider`).
 *
 * Returns ALL statuses (PENDING/CONFIRMED/IN_PROGRESS/COMPLETED/CANCELLED) so the
 * provider sees their full booking pipeline — pending requests to confirm, upcoming
 * confirmed work, and history. The endpoint is JWT-scoped server-side: the provider id
 * comes from the token, and the backend joins bookings whose service.providerId matches.
 * There's no providerId param to pass.
 */
export async function listProviderBookings(
  params: ListProviderBookingsParams = {},
): Promise<ProviderBookingPage> {
  const { page = 0, size = 10 } = params;
  return apiGet("/api/bookings/provider", {
    query: { page, size },
  }) as Promise<ProviderBookingPage>;
}
 ...
 *   - 403 not the provider who owns this booking's service (defense in depth — the UI
 *     only shows these buttons to providers, and only the owning provider may advance)
 ...
 *
 * Transitions status IN_PROGRESS → COMPLETED. This is the final provider-side step, and
