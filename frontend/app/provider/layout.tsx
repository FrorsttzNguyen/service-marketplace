...
/**
 * Segment metadata for `/provider/*`. The pages under this route are client islands
 * (React Query + form state), so static metadata is declared here in a server
 * `layout.tsx` and merged with the root template.
 *
 * `robots: { index: false, follow: false }` keeps provider-only routes out of search
 ...
 *
 * Note: segment layouts cascade, so a per-page `layout.tsx` under `/provider/services`
 * or `/provider/bookings` is not required for the noindex goal — this covers both. The
