...
/**
 * Provider — incoming bookings (`/provider/bookings`).
 *
 * The provider drives the booking lifecycle forward through three bodyless PUTs, plus a
 * paginated list of every booking on their services (all statuses):
 *   - GET    /api/bookings/provider?page&size  (list, ALL statuses)
 ...
 * rows are read-only. Completing is the final provider step and is also what unblocks the
 * customer to leave a review (a review requires a COMPLETED booking).
 *
 * Gated two ways (mirrors /provider/services + /admin/providers):
 ...
 *     endpoints require the booking's service to belong to this provider). 403 is the real
 *     security boundary.
 *
 ...
 * The row component is provider-specific (it shows the CUSTOMER name and offers lifecycle
 * actions), so it lives in this file rather than reusing the customer <BookingCard> —
 * same data shape, different actions + emphasis.
 *
 * Visual (Phase 7): island list of provider booking rows; status uses BookingStatusBadge;
...
import {
  ...
  useStartBooking,
  useProviderBookings,
} from "@/lib/api/provider-bookings-queries";
...
function ProviderBookingsContent() {
  ...
  const { data, isPending, isError, error, refetch, isFetching } =
    useProviderBookings({ page, size: PAGE_SIZE });
...
/**
 * Decide the single action (if any) a provider can take on a booking given its status.
...
function actionForStatus(
  ...
  switch (status) {
    ...
    // COMPLETED + CANCELLED are terminal — no provider action. Read-only.
    default:
      return null;
  }
}
    ...
    <Card as="li" padded className="py-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          {/*
            For the provider, the CUSTOMER is the "other party" (vs. the customer view,
            which shows the provider). Lead with the customer name so the provider knows who
            the request is from.
          */}
