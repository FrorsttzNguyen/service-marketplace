...
/**
 * Provider — service management (`/provider/services`).
 *
 * The backend's five provider-service endpoints power this single page:
 *   - GET    /api/provider/services?page&size            (list, ALL statuses)
 *   - POST   /api/provider/services                      (create → DRAFT)
 *   - PUT    /api/provider/services/{id}                 (update fields)
 *   - POST   /api/provider/services/{id}/activate        (→ ACTIVE, published)
 *   - DELETE /api/provider/services/{id}                 (→ INACTIVE, soft delete)
 * Plus the public `GET /api/categories` for the form's category dropdown.
 *
 * Gated two ways (mirrors /admin/providers):
 *   - Client route guard: <RequireAuth requireRole="VENDOR"> redirects non-providers home
 *     (and the unauthenticated to /login). Depends on the /me-in-rehydrate wiring so
 *     `user.role` survives a reload (a reloaded provider would otherwise look role-less
 *     and get bounced).
 ...
 * backend constraints) that creates a DRAFT, after which the provider can Activate to
 * publish. All mutations invalidate the provider-services family so the list refetches.
 *
...
import {
  useActivateProviderService,
  useCreateProviderService,
  useDeactivateProviderService,
  useUpdateProviderService,
  useProviderServices,
} from "@/lib/api/provider-services-queries";
import type {
  PricingType,
...
function ProviderServicesContent() {
  ...
  const { data, isPending, isError, error, refetch, isFetching } =
    useProviderServices({ page, size: PAGE_SIZE });
  ...
  // right row, not globally. Mirrors the admin/providers + bookings pages.
  ...
  const [formState, setFormState] = useState<
    null | { mode: "create" } | { mode: "edit"; service: ProviderService }
  >(null);

  const activateMutation = useActivateProviderService();
  const deactivateMutation = useDeactivateProviderService();
  ...
  function runStatusAction(
    ...
    mutation.mutate(serviceId, {
      onSuccess: () => {
        ...
        // The hook invalidates the provider-services family → the list refetches with the
  ...
  return (
    <Container width="default">
      ...
      {/*
        The form panel. Rendered above the list so the provider can create/edit without
  ...
  return (
    <Card as="li" padded className="py-5">
      ...
      {/*
        ...
        status action is in flight for THIS row (per-row gate, matching admin/providers).
      */}
...
interface ServiceFormPanelProps {
  mode: { mode: "create" } | { mode: "edit"; service: ProviderService };
  onClose: () => void;
}
...
function ServiceFormPanel({ mode, onClose }: ServiceFormPanelProps) {
  ...
  const createMutation = useCreateProviderService();
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (isEdit && editing) {
      const body: ServiceUpdateRequest = {
        ...
        // durationHours is optional — only send when the provider provided one.
        durationHours:
  ...
  return (
    ...
    >
      ...
      */}
      ...
      {/*
        Hint under the buttons: explain the create→activate flow so the provider knows a
